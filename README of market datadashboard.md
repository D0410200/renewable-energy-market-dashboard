...
1. Get a free API key from [Alpha Vantage](https://www.alphavantage.co/) or
   use `yfinance` (Python) for stock/financial data on public renewable
   energy companies (e.g. tickers like `FSLR`, `ENPH`, `ORSTED.CO`).
2. Write a small Python script using `pandas` to pull the data and reshape
   it into the same JSON shape used in `market-dashboard.jsx`
   (see the `marketGrowth`, `competitors`, and `regionalRisk` arrays at
   the top of the file).
3. Export that as a `data.json` file and import it into the component
   instead of the hard-coded arrays.

This step is optional for the portfolio piece itself, but doing it
turns this from "a nice chart" into "a data pipeline I built" —
worth including if you have an extra weekend.
